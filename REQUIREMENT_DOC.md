...
* **Historical tracking shipped (Phase 5)** — Celery worker + beat
  ...
  generalized to optionally scope to an influencer (snapshot store).
* **AI insights via Groq (Phase 8)** — Natural-language insights and
  budget recommendations are generated by **Groq** (not Claude — chosen by
  the user), default model `llama-3.3-70b-versatile`, configured via
  `GROQ_API_KEY`. Disabled with a clear message when no key is set.
